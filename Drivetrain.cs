...
public class Drivetrain : MonoBehaviour
{
	...
	void FixedUpdate()
	{
		...
		{
			float drivetrainFraction = 1.0f / wheels.Length;
			float averageAngularVelo = 0;
			for (int i = 0; i < 4; i++)
			{
				if (i == 1 || i == 0)
				{
					//2 multiplied for 2wheel drive
					averageAngularVelo += wheels[i].angularVelocity * drivetrainFraction*2;
				}
			}
            ...
            { 
				...
				float lockingTorque = (averageAngularVelo - wheels[i].angularVelocity) * differentialLockCoefficient;
				wheels[i].drivetrainInertia = inertia * drivetrainFraction;
				wheels[i].driveFrictionTorque = engineFrictionTorque * Mathf.Abs(ratio) * drivetrainFraction;
				Drivetorque = engineTorque* ratio *drivetrainFraction + lockingTorque;
				if (i == 1||i == 0)
                {
					wheels[i].driveTorque = Drivetorque;
					slipRatio += wheels[i].slipRatio * drivetrainFraction*2;
				}
